...
const STACK = ['Go', 'React', 'TypeScript', 'PostgreSQL', 'Redis', 'Docker', 'Tailwind CSS', 'Vite', 'JWT', 'REST API']

export function LandingPage() {
  const { t } = useTranslation()

  const features = [
    { icon: Blocks,   title: t('landing.features.builder.title'),  desc: t('landing.features.builder.desc'),  color: 'text-accent',  tag: 'builder'  },
    { icon: Activity, title: t('landing.features.metrics.title'),  desc: t('landing.features.metrics.desc'),  color: 'text-ember',   tag: 'metrics'  },
    { icon: Rocket,   title: t('landing.features.deploy.title'),   desc: t('landing.features.deploy.desc'),   color: 'text-success', tag: 'deploy'   },
    { icon: Shield,   title: t('landing.features.security.title'), desc: t('landing.features.security.desc'), color: 'text-info',    tag: 'security' },
    { icon: Database, title: t('landing.features.codegen.title'),  desc: t('landing.features.codegen.desc'),  color: 'text-accent',  tag: 'codegen'  },
    { icon: Globe,    title: t('landing.features.i18n.title'),     desc: t('landing.features.i18n.desc'),     color: 'text-ember',   tag: 'i18n'     },
  ]

  const steps = [
    { num: '01', title: t('landing.steps.design.title'),   desc: t('landing.steps.design.desc'),   icon: Blocks,   color: 'text-accent'  },
    { num: '02', title: t('landing.steps.generate.title'), desc: t('landing.steps.generate.desc'), icon: Terminal, color: 'text-ember'   },
    { num: '03', title: t('landing.steps.deploy.title'),   desc: t('landing.steps.deploy.desc'),   icon: Rocket,   color: 'text-success' },
  ]
  ...
  return (
    <div className="min-h-screen bg-bg-root text-text-primary">
      ...
      <section id="features" className="border-t border-edge bg-bg-surface/30 py-24">
        <div className="mx-auto max-w-5xl px-6">
          <div className="mb-14">
            ...
            <p className="mt-3 text-text-secondary max-w-xl">{t('landing.features.subtitle')}</p>
          </div>
          <div className="divide-y divide-edge border-t border-b border-edge">
            {features.map((f, i) => (
              <div key={f.tag} className="group flex items-start gap-5 py-5 hover:bg-bg-raised/20 transition-colors -mx-6 px-6 cursor-default">
                <span className="font-mono text-xs text-text-muted/60 mt-0.5 w-5 shrink-0 tabular-nums">{`0${i + 1}`}</span>
                <f.icon className={`size-4 mt-0.5 shrink-0 ${f.color}`} />
                <span className="text-sm font-semibold w-44 shrink-0 leading-tight">{f.title}</span>
                <span className="text-sm text-text-secondary leading-relaxed flex-1 hidden sm:block">{f.desc}</span>
                <span className="font-mono text-[10px] text-text-muted/30 shrink-0 mt-0.5 hidden md:block">{f.tag}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24">
        <div className="mx-auto max-w-5xl px-6">
          <div className="mb-14 text-center">
            ...
            <p className="mt-3 text-text-secondary">{t('landing.howItWorks.subtitle')}</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-edge border border-edge rounded-2xl overflow-hidden">
            {steps.map((step) => (
              <div key={step.num} className="relative flex flex-col gap-4 p-8 bg-bg-surface/30 hover:bg-bg-raised/20 transition-colors">
                <span className="absolute top-5 right-6 font-display font-bold text-[4rem] leading-none text-text-muted/5 select-none pointer-events-none">{step.num}</span>
                <div className={`flex size-10 items-center justify-center rounded-xl bg-bg-raised border border-edge group-hover:border-edge-strong ${step.color}`}>
                  <step.icon className="size-5" />
                </div>
                <div>
                  <p className="font-mono text-xs text-text-muted mb-2">{step.num}</p>
      ...
      <section className="border-t border-edge py-16 overflow-hidden">
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="absolute inset-y-0 left-0 w-24 bg-gradient-to-r from-bg-root to-transparent pointer-events-none" />
          <div className="absolute inset-y-0 right-0 w-24 bg-gradient-to-l from-bg-root to-transparent pointer-events-none" />
        </div>
      </section>
